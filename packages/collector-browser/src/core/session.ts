...
export class Session {
  ...
  public isSessionClose() {
    const sessionStartTime = (Number(cache.storage.get('sessionStartTime')) << 0) / 1000
    const updatedTime = (Number(cache.storage.get('updatedTime'))) / 1000
    const nowDateTimeSe = (this.getTime()) / 1000
    const isClose = sessionStartTime === 0 || nowDateTimeSe > updatedTime + 60 * Number(cache.config.sessionIntervalMinus) || ''
    if (isClose) {
      // 当会话首次开始时，不用发送会话关闭事件
  ...
  public previousStayTimeUpdate(hasDataType: boolean) {
    return hasDataType ? Number(cache.storage.get('previousStayTime')) : this.getTime() - Number(cache.storage.get('updatedTime'))
  }
  ...
  public sessionReferrerReset(eventName: string) {
    if (!cache.router.isSpa) {
      if ([SYSTEM_EVENTS.ACTIVATE, SYSTEM_EVENTS.SESSION_CLOSE].indexOf(eventName) > 0) {
        cache.storage.set({
          sessionReferrer: document.URL
        })
      }
  ...
  public eventEnd() {
    ...
    if (![SYSTEM_EVENTS.SESSION_START, SYSTEM_EVENTS.SESSION_CLOSE, SYSTEM_EVENTS.ACTIVATE].includes(eventName)) {
      this.sessionRestart(cache.dataType)
    }

    // 保存最后一次用户触发事件（除了会话事件以外）的事件id以及时间，通过这个时间确定会话关闭时的时间
    if (![SYSTEM_EVENTS.SESSION_START, SYSTEM_EVENTS.SESSION_CLOSE].includes(eventName)) {
      cache.storage.set({
        LASTEVENT: {
          eventName,
          time: cache.time
