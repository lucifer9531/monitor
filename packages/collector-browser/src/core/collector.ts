...
import { IBrowserConfig } from '../config/plugin'
...
export class Collector {
  ...
  constructor(token: string, config?: IBrowserConfig) {
    // 防止多实例
    if ((window as any).browserCollector) {
      return (window as any).browserCollector
    }
    cache.config = { ...config, token }
    // 创建全局事件监听者
    cache.monitor = new Monitor()
    cache.storage = new Storage(token)
    ;(window as any).browserCollector = this
  }
